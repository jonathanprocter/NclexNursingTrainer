...
export const voiceCommandSchema = z.object({
  ...
  context: z.object({
    ...
    tutorPersonality: z
      .enum(["encouraging", "analytical", "socratic", "practical"])
      .optional(),
    learningStyle: z
...
export class VoiceAssistantService {
  ...
  constructor() {
    ...
    if (!openaiApiKey) {
      throw new Error(
        "OpenAI API key is required for personalized learning assistance",
      );
    }
  ...
  async processCommand(
    ...
    try {
      ...
      if (validatedInput.context.tutorPersonality) {
        return await this.getPersonalizedResponse(
  ...
  private detectEmotionalState(
    command: string,
  ): "stressed" | "confident" | "neutral" | null {
  ...
  private generateEmotionalResponse(
    state: "stressed" | "confident" | "neutral",
  ): VoiceResponse {
    const responses: Record<
      typeof state,
      Omit<VoiceResponse, "confidence" | "emotionalSupport">
    > = {
      stressed: {
...
Provide detailed, encouraging responses with specific examples and mnemonics when applicable.`,
        ...
        practiceQuestions: [
          {
            ...
            explanation:
              "Consider the nursing process and prioritize patient safety.",
          },
        ],
      };
    } catch (error) {
  ...
  private async handleDefaultCommand(
    ...
    try {
      ...
      // Fallback default response when a more personalized study path is not available
      const defaultTopics = [
        "Fundamentals",
        "Pharmacology",
