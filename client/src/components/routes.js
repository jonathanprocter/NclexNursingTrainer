...
const study_guide_1 = __importDefault(require("./routes/study-guide"));
...
function registerRoutes(app) {
    ...
    // Study guide routes
    app.use('/api/study-guide', study_guide_1.default);
    ...
    app.get("/api/modules", async (req, res) => {
        try {
            ...
            const allModules = await _db_1.db.select().from(schema_1.modules).orderBy(schema_1.modules.orderIndex);
            if (!allModules) {
    ...
    app.post("/api/study-buddy/start", async (req, res) => {
        try {
            ...
            await _db_1.db.insert(schema_1.studyBuddyChats).values({
                userId: studentId,
                sessionId,
                role: 'assistant',
                content: message,
    ...
    app.get("/api/questions/:moduleId", async (req, res) => {
        try {
            const moduleQuestions = await _db_1.db.query.questions.findMany({
                where: (0, drizzle_orm_1.eq)(schema_1.questions.moduleId, parseInt(req.params.moduleId)),
            });
    ...
    app.post("/api/quiz-attempts", async (req, res) => {
        try {
            ...
            const newAttempt = await _db_1.db.insert(schema_1.quizAttempts).values({
                userId,
                moduleId,
                type,
                answers,
            ...
            await _db_1.db.update(schema_1.userProgress)
                .set({
                completedQuestions: schema_1.userProgress.completedQuestions + answers.length,
                correctAnswers: schema_1.userProgress.correctAnswers + answers.filter((a) => a.correct).length,
    ...
    app.get("/api/progress/:userId", async (req, res) => {
        try {
            const progress = await _db_1.db.query.userProgress.findMany({
                where: (0, drizzle_orm_1.eq)(schema_1.userProgress.userId, parseInt(req.params.userId)),
                with: {
                    module: true,
                },
            });
    ...
    app.get("/api/analytics/user/:userId", async (req, res) => {
        try {
            const attempts = await _db_1.db.query.quizAttempts.findMany({
                where: (0, drizzle_orm_1.eq)(schema_1.quizAttempts.userId, parseInt(req.params.userId)),
            ...
            const overallAnalysis = await analyzePerformance(attempts.flatMap((a) => a.answers));
            res.json({
                attempts,
                progress,
...
async function analyzePerformance(answers) {
    try {
        const completion = await openai.chat.completions.create({
            model: "gpt-4",
            messages: [
...
async function generateNewQuestions(userId, examType) {
    try {
